...
* **Works with *self-documented CSV's**: `NEAD Validator` is designed to work with self-documented CSV formats like NEAD/iCSV where metadata are imbeded in the file header. `NEAD_Validator` further checks for correspondence between the metadata and the data, and then uses that information to
 check the data.
